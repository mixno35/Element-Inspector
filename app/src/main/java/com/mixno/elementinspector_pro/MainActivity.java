...
import com.mixno.elementinspector_pro.adapter.WebEIConsoleAdapter;
import com.mixno.elementinspector_pro.app.Data;
import com.mixno.elementinspector_pro.app.EditSearchManager;
import com.mixno.elementinspector_pro.app.HistoryManager;
import com.mixno.elementinspector_pro.app.JSManager;
import com.mixno.elementinspector_pro.app.ProxySettings;
import com.mixno.elementinspector_pro.code.request.GET;
import com.mixno.elementinspector_pro.code.request.POST;
import com.mixno.elementinspector_pro.dialog.AddonsDialog;
import com.mixno.elementinspector_pro.dialog.CodeDialog;
import com.mixno.elementinspector_pro.dialog.DownloadFileDialog;
import com.mixno.elementinspector_pro.dialog.LoadHTMLDialog;
import com.mixno.elementinspector_pro.dialog.RateAppDialog;
import com.mixno.elementinspector_pro.dialog.UserAgentDialog;
import com.mixno.elementinspector_pro.dialog.WelcomeDialog;
import com.mixno.elementinspector_pro.menu.AnchorMenu;
import com.mixno.elementinspector_pro.menu.ImageMenu;
import com.mixno.elementinspector_pro.model.BackHistoryModel;
import com.mixno.elementinspector_pro.model.ConsoleModel;
import com.mixno.elementinspector_pro.web.JavaScriptInterface;
import com.mixno.elementinspector_pro.web.SSL;
import com.mixno.elementinspector_pro.web.WebEIConsole;
...
public class MainActivity extends AppCompatActivity {
    ...
    private LinearLayout bottomSheetConsole;
    ...
    private BottomSheetBehavior bottomSheetConsoleMain;
    ...
    private boolean ERUDA_LOADED = false;
...
//            mWeb.runJS("console.log('1:"+str+" 2:"+str2+" 3:"+str3+"');");
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        bottomSheetConsole = findViewById(R.id.bottomSheetConsole);
        ...
        bottomSheetConsoleMain = BottomSheetBehavior.from(bottomSheetConsole);

        listConsole = findViewById(R.id.recyclerViewConsole);
        ...
        bottomSheetConsoleMain.setBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            @Override
            ...
            public void onSlide(@NonNull View bottomSheet, float slideOffset) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        linBottomMenu.setAlpha(Data.reverseNum(String.format("%.1f", slideOffset)));
                    }
                });
...
//                    @Override
//                    public void run() {
//                        ((TextView)findViewById(R.id.textConsole)).setText(Data.reverseNum(String.format("%.1f", slideOffset)) + "");
//                    }
//                });
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (String.format("%.1f", slideOffset).equals("1,0")) {
        ...
        ((Button)findViewById(R.id.buttonMainJSRun)).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        ((Button)findViewById(R.id.buttonMainJSClear)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (consoleList.size() > 0) {
                    ...
                    consoleERROR = 0;
                    consoleLOG = 0;
                    ((TextView)findViewById(R.id.textConsoleWarning)).setText(String.valueOf(consoleWARNING));
                    ((TextView)findViewById(R.id.textConsoleError)).setText(String.valueOf(consoleERROR));
                    ((TextView)findViewById(R.id.textConsoleLog)).setText(String.valueOf(consoleLOG));
                    consoleListFun(consoleList);
                } else {
                    Toast.makeText(MainActivity.this, "No content console for clear!", Toast.LENGTH_SHORT).show();
                }
            }
        });
...
//        Toast.makeText(this, this.getLocalClassName(), Toast.LENGTH_SHORT).show();
        ...
        if (shared.getBoolean("KeyProxySettingsBool", false)) {
            try {
                if (ProxySettings.setProxyWebView(mWeb, shared.getString("KeyProxySettingsHost", ProxySettings.PROXY_DEFAULT_HOST), shared.getInt("KeyProxySettingsPort", ProxySettings.PROXY_DEFAULT_PORT), this.getLocalClassName())) {
                    Toast.makeText(this, "Proxy started: " + shared.getString("KeyProxySettingsHost", ProxySettings.PROXY_DEFAULT_HOST), Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(this, "Proxy starting error: " + shared.getString("KeyProxySettingsHost", ProxySettings.PROXY_DEFAULT_HOST), Toast.LENGTH_SHORT).show();
                }
            } catch (Exception e) {
                Toast.makeText(this, "Error proxy starting: " + e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        }
...
//                        mWeb.runJS(JSManager.COOKIES_MANAGER);
//                        new CookieManagerDialog(MainActivity.this, mWeb);
                        startActivity(new Intent(MainActivity.this, CookieManagerActivity.class).putExtra("url", mWeb.getUrl()));
                        break;
                }
                return true;
            }
...
//                Toast.makeText(context, "Color light", Toast.LENGTH_SHORT).show();
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (linMainIncludeFindToPage.getVisibility() == View.GONE) {
                    ...
                    bottomSheetConsole.setVisibility(View.GONE);
                }
            }
        });
    }
            ...
            public void run() {
                if (linMainIncludeFindToPage.getVisibility() == View.VISIBLE) {
                    ...
                    if (shared.getBoolean("keyViewBottomMenuBar", true) == true) {
                        linBottomMenu.setVisibility(View.VISIBLE);
                        bottomSheetConsole.setVisibility(View.VISIBLE);
                    } else {
                        linBottomMenu.setVisibility(View.GONE);
                        bottomSheetConsole.setVisibility(View.GONE);
                    }
    ...
    private class WebClient extends WebViewClient {
        ...
        public void onLoadResource(WebView view, String url) {
            ...
            } if (!ERUDA_LOADED && shared.getBoolean("keyDefaultEruda", false)) {
                ERUDA_LOADED = true;
                mWeb.runJS(JSManager.ERUDA_EI);
            }
        }
...
//            }, 2000);
    ...
    private class WebChrome extends WebChromeClient {
        ...
        public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
            consoleList.add(new ConsoleModel(consoleMessage.message(), consoleMessage.lineNumber(), consoleMessage.sourceId(), new Long(System.currentTimeMillis()/1000).toString(), consoleMessage.messageLevel().name()));
            if (bottomSheetConsoleMain.getState() == 3) {
                consoleListFun(consoleList);
            }
            if (consoleMessage.messageLevel().name().equals("WARNING")) {
                consoleWARNING = consoleWARNING + 1;
                ((TextView)findViewById(R.id.textConsoleWarning)).setText(String.valueOf(consoleWARNING));
            } if (consoleMessage.messageLevel().name().equals("ERROR")) {
                consoleERROR = consoleERROR + 1;
                ((TextView)findViewById(R.id.textConsoleError)).setText(String.valueOf(consoleERROR));
            } if (consoleMessage.messageLevel().name().equals("LOG")) {
                consoleLOG = consoleLOG + 1;
                ((TextView)findViewById(R.id.textConsoleLog)).setText(String.valueOf(consoleLOG));
            }
            // Toast.makeText(MainActivity.this, bottomSheetConsoleMain.getState() + "", Toast.LENGTH_SHORT).show();
            // Toast.makeText(MainActivity.this, consoleMessage.messageLevel().name(), Toast.LENGTH_SHORT).show();
            return true;
        }
...
//        }
    ...
    private void consoleListFun(ArrayList<ConsoleModel> list) {
        try {
            listAdapter = new WebEIConsoleAdapter(list, this);
            listConsole.setAdapter(listAdapter);
        } catch (Exception e) {
            Toast.makeText(this, "Error loading list console: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
}
